...
pub enum ContractError {
    ...
    // ── Ownership Mirror / Two-Step Transfer Errors (SC-090) ─────────────
    /// Contract owner has not been initialized.
    OwnerNotSet = 42,
    /// No pending ownership transfer is currently in flight.
    NoPendingOwner = 43,
    /// The pending transfer is still within the minimum delay window.
    OwnershipTransferTooEarly = 44,
    /// The proposed owner is already the active owner.
    InvalidOwner = 45,
    /// Source contract for mirrored ownership has not been configured.
